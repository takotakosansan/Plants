class Customer < ApplicationRecord
  ...
  def posts_with_reposts
    relation = Post.joins("LEFT OUTER JOIN reposts ON posts.id = reposts.post_id AND reposts.customer_id = #{id}").
      select("posts.*, reposts.customer_id, (SELECT name FROM customers WHERE id = reposts.customer_id) AS repost_customer_name")
    relation.where(customer_id: id).
      or(relation.where("reposts.customer_id = ?", id)).
      with_attached_image.
